...
module Api
  module V1
    class RecommendationsController < ApplicationController
      ...
      def uniform
        ...
        sections = build_uniform_sections(school_id, nearby_ids, uniform_cat, gender, min_price, max_price)
      ...
      def sport
        ...
        sections = build_sport_sections(school_id, nearby_ids, sport_cat, sport_type, min_price, max_price)
      ...
      def with_all_associations(scope)
        scope.includes(:main_category, :gender, :item_condition, :brand, :school)
      end
 ...
 def recommended_items_with_fallback(school_id, nearby_ids)
  ...
  if preferred_categories.any?
    # Main items with images (with CAST)
    ...
    if items.count >= 4
      ...
      # ✅ FIXED: Added ::text cast here too
  ...
  popular_with_fallback(school_id, nearby_ids)
      ...
      def build_uniform_sections(school_id, nearby_ids, uniform_cat, gender, min_price = nil, max_price = nil)
        base_query = with_all_associations(Item).where(main_category_id: uniform_cat).where(deleted: false, status: 'active')
      ...
      def build_sport_sections(school_id, nearby_ids, sport_cat, sport_type, min_price = nil, max_price = nil)
        base_query = with_all_associations(Item).where(main_category_id: sport_cat).where(deleted: false, status: 'active')
  ...
  items.map do |item|
      ...
      begin
        if additional.is_a?(String) && additional.start_with?('[')
          ...
          if parsed.is_a?(Array)
            parsed.each { |img| image_urls << img if img.present? && !image_urls.include?(img) }
          end
        elsif additional.is_a?(Array)
          additional.each { |img| image_urls << img if img.present? && !image_urls.include?(img) }
        else
...

    {
      ...
      price: actual_price,  # ← Use variant price
